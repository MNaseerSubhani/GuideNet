...
class FeatureMLP(nn.Module):
    # keep this but it's good to make default flexible (I leave default untouched).
...
class Denoiser(nn.Module):
    def __init__(self, embed_dim=1280):   # <-- add embed_dim arg
    ...
    def forward(self, noisy_tensor, roadgraph_tensor, feature_mask, roadgraph_mask):
        ...
        noisy_tensor = self.feature_mlp(noisy_tensor)  # no change: input matches updated size
        x = noisy_tensor
